...
public class RestController {
    ...
    private ResponseEntity<Response> get(Request request) {
        ...
        return nonNull(value) ? ResponseEntity.ok(new Response(request.getKey(), value)) : notFound().build();
    }

    private ResponseEntity<Response> put(Request request) {
        dhtService.put(request.getKey(), request.getValue());
        return ResponseEntity.ok(new Response(request.getKey(), request.getValue()));
    }
}